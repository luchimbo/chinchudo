...
def instagram_target_url(row: dict) -> str:
    url = row.get("source_thread_url") or row.get("source_url") or row.get("published_url") or ""
    return url if re.search(r"instagram\.com/(p|reel)/", url) else ""


def reusable_target_key(row: dict, channel: str) -> str:
    source_url = row.get("source_thread_url") or row.get("source_url") or row.get("published_url") or ""
    if channel == "instagram":
        source_url = instagram_target_url(row)
    if source_url:
        return f"{channel}:url:{source_url}"
    landing = row.get("landing_url") or row.get("landing_slug") or ""
    title = row.get("title") or row.get("source_thread_title") or ""
    body = row.get("body") or ""
    signature = hashlib.sha1(f"{landing}|{title}|{body[:240]}".encode("utf-8")).hexdigest()
    return f"{channel}:content:{signature}"
...
def used_targets_by_channel(log: list[dict]) -> set[str]:
    used: set[str] = set()
    for row in log:
        published_channels = row.get("auto_published_channels") or {}
        attempted_result = row.get("auto_publish_result") or {}
        attempted_channel = normalize_channel(row.get("channel", ""))
        channels = set()
        if isinstance(published_channels, dict):
            channels.update(normalize_channel(ch) for ch in published_channels)
        if row.get("status") == "published" and attempted_channel:
            channels.add(attempted_channel)
        if row.get("auto_publish_attempted_at_utc") and attempted_channel:
            channels.add(attempted_channel)
        for channel in channels:
            if not channel:
                continue
            key = reusable_target_key(row, channel)
            if key:
                used.add(key)
            result_url = attempted_result.get("url")
            if result_url:
                used.add(f"{channel}:url:{result_url}")
    return used
...
def distribution_candidates(channels: set[str], limit: int, per_channel: int, allow_proposed: bool) -> tuple[list[dict], list[dict]]:
    ...
    used_targets = used_targets_by_channel(rows)
    ...
    for index, row in reversed(list(enumerate(rows))):
        ...
        for channel in target_channels:
            if channel_already_published(row, channel):
                continue
            target_key = reusable_target_key(row, channel)
            if target_key in used_targets or target_key in selected_targets:
                blocked.append({"index": index, "entry": row, "channel": channel, "errors": ["Destino/contenido ya usado en este canal"]})
                continue
            if channel == "instagram":
                target_url = instagram_target_url(row)
                if not target_url:
                    blocked.append({"index": index, "entry": row, "channel": channel, "errors": ["Instagram requiere URL concreta de post/reel"]})
                    continue
                if f"{channel}:url:{target_url}" in used_targets or f"{channel}:url:{target_url}" in selected_targets:
                    blocked.append({"index": index, "entry": row, "channel": channel, "errors": ["Instagram: destino ya usado"]})
                    continue
            ...
            if channel == "instagram":
                selected_targets.add(f"{channel}:url:{instagram_target_url(row)}")
...
def post_url_for_entry(entry: dict, channel: str) -> str:
    ...
    if channel == "youtube":
        ...
        return url
    if channel == "instagram":
        url = instagram_target_url(entry)
        if not url:
            raise ValueError("Instagram requiere source_url/source_thread_url de un post o reel no usado")
        return url

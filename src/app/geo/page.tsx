    ...
    <main className="relative mx-auto flex min-h-screen w-full max-w-5xl flex-col px-5 py-8">
      <header className="mb-8 flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-ink">Auditoría GEO</h1>
          <p className="mt-0.5 text-sm text-slate">
            Presencia de PC MIDI Center en respuestas de IAs
          </p>
        </div>
        <div className="rounded-xl border border-ink/10 bg-paper px-5 py-3 text-center shadow-sm">
          <p className="text-xs text-slate">Score promedio</p>
          <p className={`text-3xl ${scoreColor(parseFloat(avgScore) || 0)}`}>{avgScore}</p>
          <p className="text-xs text-slate">/ 5 · {avg._count.id} audits</p>
        </div>
      </header>

      {audits.length === 0 ? (
        <p className="text-sm text-slate">
          No hay auditorías todavía. Ejecutá{" "}
          <code className="font-mono text-xs">python agents/orchestrator.py geo-audit</code>.
        </p>
      ) : (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-ink/5 px-2 py-0.5 text-xs font-mono text-slate">
                        {audit.modeloIA}
                      </span>
                    ...
                    {competidores.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-semibold text-slate">Competidores detectados:</p>
                    ...
                    {gaps.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-semibold text-slate">Gaps de contenido:</p>

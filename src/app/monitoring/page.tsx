...
async function loadAccounts(): Promise<{ id: string; label: string }[]> {
  try {
    const raw = await readFile(join(process.cwd(), "agents", "accounts.json"), "utf-8");
    const data = JSON.parse(raw) as Record<string, { label?: string }>;
    return Object.entries(data).map(([id, cfg]) => ({ id, label: cfg.label ?? id }));
  } catch {
    return [];
  }
}
...
export default async function MonitoringPage() {
  ...
  const [sources, recent, accounts] = await Promise.all([
    prisma.monitoredSource.findMany({ orderBy: { label: "asc" } }),
    prisma.opportunity.findMany({
      where: { monitoredSourceId: { not: null }, createdAt: { gte: since } },

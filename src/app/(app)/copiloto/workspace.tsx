...
function ResponseCard({ response, opportunityId, sourceUrl }: { response: Response; opportunityId: string; sourceUrl: string }) {
  ...
  async function openForPublishing() {
    ...
    // Nunca navegar la pestaña actual: si el popup se bloquea, se muestra un link manual.
  ...
  return <div className={`rounded-xl border p-4 ${response.isPrimary ? "border-moss/45 bg-moss/[0.05]" : "border-ink/10 bg-white"}`}>
    <form action={markCopilotResponse}>
      ...
      {popupBlocked ? <p className="mt-3 rounded-lg border border-brass/30 bg-brass/10 px-3 py-2 text-xs text-ink">El navegador bloqueó la pestaña nueva. La respuesta ya quedó copiada: <a href={sourceUrl} target="_blank" rel="noreferrer" className="font-bold underline underline-offset-4">abrí el post desde acá</a>.</p> : null}
    </form>

...
export function CopilotWorkspace({ activeClient, youtube, activeView, filters, pulse, opportunities }: { activeClient: { slug: string; name: string } | null; youtube: { account: string; connected: boolean; channelTitle: string } | null; activeView: "opportunities" | "pulse"; filters: { brands: { id: string; name: string }[]; channels: { id: string; name: string }[]; selectedBrand: string; selectedChannel: string; selectedResponse: string; selectedSort: string }; pulse: PulseSignal[]; opportunities: Opportunity[] }) {

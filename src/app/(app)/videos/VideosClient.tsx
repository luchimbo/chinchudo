...
export default function VideosClient({ activeClient, clients, trends, products, personas, scripts, ideas }: Props) {
  ...
  return <div className="min-h-screen bg-paper text-ink"><main className="mx-auto w-full max-w-7xl px-5 py-7">
    <header className="relative overflow-hidden rounded-2xl border border-ink/10 bg-ink px-6 py-7 text-paper shadow-panel">
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[.22em] text-brass">Estudio editorial</p><h1 className="mt-2 font-display text-4xl font-bold">Ideas para grabar</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-paper/75">Elegí qué producto mover. Después elegí una idea que se pueda filmar.</p></div><select value={activeClient.slug} onChange={(event) => router.push(`/videos?client=${event.target.value}`)} className="h-10 rounded-md border border-paper/20 bg-white/10 px-3 text-sm font-bold text-paper">{clients.map((client) => <option className="text-ink" key={client.id} value={client.slug}>{client.name}</option>)}</select></div>
    ...
    {tab === "create" && <section className="mt-7 grid gap-6 lg:grid-cols-[.9fr_1.4fr]"><div className="rounded-xl border border-ink/10 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-wider text-brass">1. Punto de partida</p><h2 className="mt-2 font-display text-2xl font-bold">¿Qué queremos mover?</h2><label className="mt-5 grid gap-2 text-sm font-bold">Producto<select value={productId} onChange={(event) => setProductId(event.target.value)} className="h-11 rounded-md border border-ink/15 bg-paper px-3">{products.map((product) => <option key={product.id} value={product.id}>{product.name} · {product.brand.name}</option>)}</select></label>{currentProduct && <p className="mt-3 rounded-md bg-moss/10 p-3 text-sm leading-6 text-slate">{currentProduct.description}</p>}<p className="mt-6 text-xs font-black uppercase tracking-wider text-brass">2. Intención</p><div className="mt-3 grid gap-2">{INTENTS.map((option) => <button key={option.value} onClick={() => setIntent(option.value)} className={`rounded-lg border p-3 text-left ${intent === option.value ? "border-brass bg-brass/10" : "border-ink/10 hover:bg-paper"}`}><span className="block text-sm font-black">{option.label}</span><span className="mt-1 block text-xs text-slate">{option.hint}</span></button>)}</div><button disabled={loading || !productId} onClick={generateAngles} className="mt-6 h-11 w-full rounded-md bg-ink text-sm font-black text-paper hover:bg-ink/90 disabled:opacity-50">{loading ? "Buscando ideas…" : "Proponer 3 ideas"}</button></div>
      <div><div className="mb-4"><p className="text-xs font-black uppercase tracking-wider text-brass">3. Elegir una dirección</p><h2 className="mt-1 font-display text-2xl font-bold">Ideas para este producto</h2></div>{angles.length === 0 ? <Empty title="Primero una idea, después un guion" body="Elegí producto e intención. Vas a recibir tres maneras concretas de mostrarlo."/> : <div className="grid gap-4 xl:grid-cols-3">{angles.map((angle, index) => <article key={`${angle.hook}-${index}`} className="flex min-h-[330px] flex-col rounded-xl border border-ink/10 bg-white p-5 shadow-sm"><span className="w-fit rounded bg-moss/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-moss">{angle.format}</span><h3 className="mt-5 font-display text-xl font-bold leading-tight">“{angle.hook}”</h3><Detail label="Por qué funciona" text={angle.rationale}/><Detail label="Cómo se graba" text={angle.visualDirection}/><button disabled={saving} onClick={() => approveAngle(angle)} className="mt-auto h-10 rounded-md bg-ink text-xs font-black text-paper hover:bg-ink/90">Usar esta idea</button></article>)}</div>}</div></section>}

    {tab === "references" && <section className="mt-7"><p className="text-xs font-black uppercase tracking-wider text-brass">Referencias públicas</p><h2 className="mt-1 font-display text-2xl font-bold">Videos que están funcionando</h2><p className="mt-2 text-sm text-slate">Mirá el video, entendé qué hace y usalo como punto de partida.</p><div className="mt-5 grid gap-4 lg:grid-cols-2">{trends.map((trend) => <article key={trend.id} className="rounded-xl border border-ink/10 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center gap-2"><span className="rounded bg-moss/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-moss">{referenceType(trend.platform)}</span><span className="text-[10px] font-black uppercase tracking-wider text-brass">{PLATFORM[trend.platform] || trend.platform}</span></div><h3 className="mt-3 text-lg font-black leading-tight">{trend.title}</h3>{trend.analysisStatus === "ANALYZED" ? <div className="mt-4 grid gap-3 rounded-lg bg-paper p-4"><Detail label="Qué pasa" text={trend.description}/><Detail label="Por qué llama la atención" text={trend.viralFormula}/><Detail label="Cómo usarlo con un producto" text={trend.suggestedAngle}/><Detail label="Cómo grabarlo" text={trend.visualDirection}/><div className="flex flex-wrap gap-2 pt-1">{trend.sourceUrl && <a href={trend.sourceUrl} target="_blank" rel="noreferrer" className="rounded-md border border-ink/15 px-3 py-2 text-xs font-black">Ver video</a>}<button onClick={() => useReference(trend)} className="rounded-md bg-ink px-3 py-2 text-xs font-black text-paper">Usar esta idea</button></div></div> : <div className="mt-4"><p className="line-clamp-3 text-sm leading-6 text-slate">{trend.description}</p><button disabled={saving} onClick={() => prepareReference(trend.id)} className="mt-4 rounded-md border border-ink/15 px-3 py-2 text-xs font-black">{saving ? "Preparando…" : "Preparar esta referencia"}</button></div>}</article>)}</div>{trends.length === 0 && <Empty title="Todavía no encontramos videos" body="El radar busca videos y formatos públicos para mostrarte nuevas ideas."/>}</section>}

...
export default function VideosClient({ activeClient, clients, trends, products, personas, scripts, ideas }: Props) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-paper text-ink"><main className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-5 sm:py-7">
    ...
    <nav aria-label="Secciones de tendencias y guiones" className="-mx-4 mt-5 flex gap-1 overflow-x-auto border-b border-ink/10 px-4 sm:mx-0 sm:mt-6 sm:px-0">{([ ["create", "Crear idea"], ["references", "Videos que funcionan"], ["production", "Producción"] ] as [Tab, string][]).map(([id, label]) => <button key={id} onClick={() => setTab(id)} className={`shrink-0 whitespace-nowrap px-3 py-3 text-sm font-black sm:px-4 ${tab === id ? "border-b-2 border-brass text-ink" : "text-slate"}`}>{label}</button>)}</nav>

...
export async function generateEditorialAngles({ clientId, productId, intent }: { clientId: string; productId: string; intent: ContentIntent }): Promise<EditorialAngle[]> {
  ...
  const prompt = `Genera exactamente 3 angulos de contenido vertical para Argentina. Cliente: ${client.name}. Rubro del cliente: ${client.description}. Producto: ${product.name} (${product.brand.name}). Descripcion: ${product.description}. Usos: ${product.useCases}. Intencion: ${INTENT_LABELS[intent]}.\nReferencias disponibles, ya relacionadas con este rubro:\n${trendContext}\n\nAdaptá los ejemplos, escenas y lenguaje al rubro del cliente. No inventes precio, stock, garantia ni especificaciones. Cada angulo debe ser grabable con manos, producto y un espacio comercial u hogareno simple. Devuelve JSON: {"angles":[{"format":"","hook":"","rationale":"","visualDirection":"","viabilityScore":1-5,"trendId":"opcional"}]}`;
  try {
    const { response } = await fetchChatCompletion(llm, { messages: [{ role: "system", content: "Devolves solamente JSON valido." }, { role: "user", content: prompt }], response_format: { type: "json_object" }, temperature: 0.8, max_tokens: 1400 }, "Los 5 Apostoles - Ideas para grabar", client);
...
export async function analyzeTrend(trendId: string) {
  ...
  const prompt = `Analiza esta referencia para contenido corto para ${client.name}, cuyo rubro es: ${client.description}. titulo=${trend.title}; descripcion=${trend.description}; plataforma=${trend.platform}. Catalogo:\n${catalog}\nDevuelve JSON {"isAudiovisual":true,"viabilityScore":1-5,"viralFormula":"una oracion","suggestedProductId":"id o vacio","suggestedAngle":"una oracion","visualDirection":"una oracion"}. Priorizá escenas, necesidades y vocabulario propios del rubro. Si es una busqueda abstracta o no tiene formato audiovisual, isAudiovisual es false.`;
  try {
    const { response } = await fetchChatCompletion(llm, { messages: [{ role: "system", content: "Devolves solamente JSON valido." }, { role: "user", content: prompt }], response_format: { type: "json_object" }, temperature: 0.2, max_tokens: 700 }, "Los 5 Apostoles - Analisis de radar", client);
